...
namespace NetDapperWebApi.Services
{
    public class HotelService : IHotelService
    {
        ...
        public async Task<string> AddCategoryDetailToHotelAsync(AddRelationsMM<int, int> dto)
        {
            ...
            parameters.Add("@CategoryDetailsJson", jsonCategories);
            var result = await _db.QueryFirstOrDefaultAsync<string>("sp_AddHotelCategoryDetails", parameters, commandType: CommandType.StoredProcedure);
            return result;
        }
        ...
        public async Task<Hotel> GetHotel(int id, int depth)
        {
            ...
            List<CategoryDetails> hotelAmenities = [];
            List<RoomType> roomTypes = [];

            if (depth >= 1)
            {
                hotelAmenities = [.. await multi.ReadAsync<CategoryDetails>()];
            ...
            if (depth >= 2)
            {
                var roomTypeAmenities = (await multi.ReadAsync<CategoryDetails>()).ToList(); // Tiện ích của RoomType
                var rooms = (await multi.ReadAsync<Room>()).ToList(); // Rooms của khách sạn

                foreach (var roomType in hotel.RoomTypes)
                {
                    roomType.Amenities = roomTypeAmenities.Where(x => x.RoomTypeId == roomType.Id).ToList();
                    roomType.Rooms = rooms.Where(r => r.RoomTypeId == roomType.Id).ToList();
                }
            }
